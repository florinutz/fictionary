...
export const inflateCommand: CommandDefinition = {
  ...
  setup(program: Command): Command {
    ...
    return program
      ...
      .option('-l, --enrichment-level <level:string>', 'Level of detail (basic, standard, detailed)', { 
        default: 'standard' 
      })
      .option('-c, --categories <categories:string[]>', 'Categories to include (world, characters, magic, etc.)')
      .option('-o, --output-file <file:string>', 'File to save the output to') as unknown as Command;
  },
  ...
  async action(options: Record<string, unknown>, ...inputs: string[]): Promise<void> {
    ...
    try {
      ...
      // Format the output
      const formattedOutput = workflow.formatOutput(result, inflateOptions.outputFormat as OutputFormat);

      // Save to file if specified
      if (inflateOptions.outputFile) {
        await Deno.writeTextFile(inflateOptions.outputFile, 
