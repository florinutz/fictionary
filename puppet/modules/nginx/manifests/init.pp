...
# Add a vhost template
  file { 'vagrant-nginx':
    path => '/etc/nginx/sites-available/fictionar.ro',
    ensure => file,
    require => Package['nginx'],
    source => 'puppet:///modules/nginx/fictionar.ro',
  }
...
# Symlink our vhost in sites-enabled to enable it
  file { 'vagrant-nginx-enable':
    path => '/etc/nginx/sites-enabled/fictionar.ro',
    target => '/etc/nginx/sites-available/fictionar.ro',
    ensure => link,
